...
def game(callBackData):
    ...
    if not buttonNumber == 9: 
        ...
        if charList[buttonNumber] == " ":
            ...
            alert = 'Нажимать можно только на пустую кнопку'
    ...
    if message == "Вы победили" or message == "Победил бот" or message == "Ничья":
        message += '\n'
        for i in range(0, 3):
            message += '\n | '
            for j in range(0, 3):
...
def help_command(update, _):
    update.message.reply_text('Бот для игры в крестики-нолики используйте команду /new_game для начала новой игры')
